...
document.addEventListener("DOMContentLoaded", () => {
  const wrapper = document.querySelector(".related__wrapper");
  const related = wrapper?.querySelector("div[data-related-section]");

  if (!wrapper || !related) return;

  const checkRelated = () => {
    const isEmpty =
      related.matches(":empty") ||
      related.innerHTML.trim() === "";
  ...
  // ⏱ Shopify async load delay
  setTimeout(checkRelated, 500);

  // 👀 Watch for late injection
  const observer = new MutationObserver(checkRelated);
